...
<?php do_action('foundationPress_post_before_entry_content'); ?>
				...
				if ($attachments) {
					foreach ($attachments as $attachment) {
						$item_thumb = wp_get_attachment_image_src($attachment->ID,'thumbnail');
						$thumb_title = apply_filters('the_title', $attachment->post_title);


						echo '<div><img src="';
						echo $item_thumb[0];
						echo '" alt="'.$thumb_title.'"/></div>';
				...
				if ($attachments) {
					foreach ($attachments as $attachment) {
						...
						echo '<div id="'.$attslug.'-item-'.$attachment->ID.'">';
